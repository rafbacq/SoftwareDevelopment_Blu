...
public class Test{
	...
	public Test() {
		...
		testFrame.setTitle("To-Do Application");
		...
		testFrame.add(inputPanel, BorderLayout.SOUTH);
		...
		fileMenu.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				int result = JOptionPane.showConfirmDialog(null, fileSystem, "Select File",
		...
		colorTheme.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				switch (color) {
				case "Love":
					testFrame.setBackground(new Color(240, 182, 213));
					break;
				case "Halloween":
					testFrame.getContentPane().setBackground(Color.BLACK);
					break;
				case "Winter":
					testFrame.getContentPane().setBackground(new Color(173, 216, 230));
					break;
				case "Dark mode":
					testFrame.getContentPane().setBackground(Color.BLACK);
					break;
				case "Pastel":
					testFrame.getContentPane().setBackground(new Color(212, 231, 197));
					break;
				case "Default":
	...
	private static void runGUI() {
	    ...
	    Test test = new Test();
	    test.testFrame.setVisible(true); // Make the frame visible
	}
	    ...
	    try {
	        ...
	        if (support.isDataFlavorSupported(DataFlavor.stringFlavor)) {
	            ...
	            if (support.getSourceDropActions() == MOVE) {
	                ...
	                int[] sourceRows = targetTable.getSelectedRows();

	                // If rows are selected
	                if (sourceRows.length > 0) {
	                    // Remove rows from original position
	                    for (int i = sourceRows.length - 1; i >= 0; i--) {
	                        targetModel.insertRow(dropRow, targetModel.getDataVector().elementAt(sourceRows[i]));
	                        if (sourceRows[i] < dropRow) {
	                            dropRow--; // Adjust dropRow if row is removed before it
	                        }
	                        targetModel.removeRow(sourceRows[i]);
	                    }
	                    return true;
	                }
	            }
